...
public class Stop_indexContextExt extends AbstractBaseExtendedContext {
	...
	public Stop_indexContext getContext(String str) {
		return (Stop_indexContext) new LAParserPopulateExtendedContextVisitor()
				.visit(((LAParser) getParser(str)).stop_index());
	}
		...
		if (ctx != null) {
			if (ctx instanceof Stop_indexContext) {
				...
				throw new ClassCastException(
						ctx.getClass().getSimpleName() + " cannot be cased to " + Stop_indexContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
	...
	public void Initialize() throws Exception {
		super.Initialize();
		// stop_index = getContext().constant_expression().extendedContext.eval();
		// //Wrong way of accessing the extended context of a child nodes.
		stop_index = ((Constant_expressionContextExt) extendedContextVisitor
